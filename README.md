...
- 模糊、小尺寸、压缩或遮挡人脸自动触发整组人物像素锁定；保留原始模糊，不允许 AI 猜测或补清五官
- 保留人物身份、景物、胶片颗粒与现场光线
...
执行前会在 200% 视图检查每张脸。若任一人脸在计划变换后的短边小于 48 个源像素，或因失焦、运动模糊、压缩、遮挡、低对比度而无法可靠区分眼鼻口，整个任务会自动切换为严格的整组人物像素锁定。原图的模糊会被原样保留，不做增强、锐化、降噪或重建。若无法完成确定性合成与零 RGB 差异验证，只能提供明确标注的非最终版式预览，不交付正式成品。
