...
Run the low-detail face safety gate before generation. If any face is smaller than 48 source pixels on its shorter side after the planned whole-photo transform, or if blur, compression, occlusion, low contrast, or distance makes facial features unreliable, switch the entire job to strict whole-person pixel lock. Preserve the source blur exactly. Do not enhance, sharpen, denoise, reconstruct, or regenerate any person. If deterministic compositing and zero-RGB-difference verification cannot be completed, do not deliver a formal finished image.
